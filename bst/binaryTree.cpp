...
int BinarySearchTree::search(int data_to_search, node * root)>
{
    if(root == NULL)
        return 0;
    ...
    {
       return search(data_to_search, root->right_child);
    }
    else if(data_to_search < root->right_child )
    {
       return search(data_to_search, root->left_child);
    }
